...
def image_route(input_layer, num_res_blocks=3, activation='leaky', dropout=0.1, init_num_features_map=32, final_feature_maps=64, use_pooling=True):
    ...
    for i in range(num_res_blocks):
        ...
        conv = common.convolutional(conv, activate_type=activation, dropout=dropout, filters_shape=(1,1, num_fm, 2*num_fm), downsample=True)
        if conv.shape[1] > 1:
            if use_pooling:
...
def PPNet(input_image, input_vector, num_fc_blocks=2, num_res_blocks=2, fc_activation='leaky', conv_activation='leaky', init_num_units=16, final_num_units=64, fc_dropout=0.5, conv_dropout=0.1):
    vector_output = vector_route(input_vector, num_fc_blocks, fc_activation, fc_dropout, init_num_units=init_num_units, final_units=final_num_units)
    image_output = image_route(input_image, num_res_blocks, conv_activation, conv_dropout, init_num_features_map=init_num_units, final_feature_maps=final_num_units)
    ...
    net_output = common.fully_connected(net_output, units=16, activate_type=fc_activation, dropout=fc_dropout)
    net_output = common.fully_connected(net_output, units=3, activate=False)

    return net_output

def create_net():
    input_vector = keras.layers.Input(shape=(4,), name='input_vector')
    input_image = keras.layers.Input(shape=(416,416,3), name='input_image')
    net_output = PPNet(input_image, input_vector)
    model = keras.Model(inputs=[input_image, input_vector], outputs=net_output, name='PPNet')
